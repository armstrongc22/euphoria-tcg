...
/**
 * The Euphoria interactive map. Normal visitors get a responsive, zoom/pan map
 * whose markers open lore popups. Notation (editor) mode is hidden: it's
 * revealed by the secret gesture of tapping the corner compass emblem 5× within
 * a few seconds (or via the ?mapDebug=1 developer fallback). Once unlocked you
 * can click to place a marker, drag to fine-tune, edit/delete, and JSON
 * import/export. Marker data lives in localStorage; the unlock flag lives in
 * sessionStorage so it never leaks to normal visitors and resets each session.
 *
...
export function InteractiveMap() {
  ...
  // Persist on every change — the set is tiny so this is cheap.
  useEffect(() => {
    saveMarkers(markers);
  }, [markers]);
