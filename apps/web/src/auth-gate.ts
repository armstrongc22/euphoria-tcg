/**
 * Full-screen Euphoria auth gate. The playable beta client is login-gated: none
 * of the game screens (splash, menu, match, deck editor, rewards, collection,
 * account) mount or render until this gate reports a real, authenticated
 * session. It uses the SAME `Auth` backend as the rest of the app (Supabase when
 * VITE_SUPABASE_* are set, else the localStorage demo) — there is no second auth
 * system here; it only drives auth.signIn / auth.signUp and hands the resulting
 * session back through `onAuthed`.
 */
 ...
 */
export function mountAuthGate(root: HTMLElement, { auth, onAuthed }: AuthGateOptions): void {
  root.innerHTML = `
  ...
  // Enter / "Sign In" → sign in to an existing account.
  ...
  // "Create Account" → register, then continue straight into the beta.
